...
@interface RXSelectPayViewController ()
@property (nonatomic, strong) UIButton *settedBtn;
@property (nonatomic, strong) UIButton *customBtn;

@end

@implementation RXSelectPayViewController
...
- (void)customBtnAction{
    RXPayPlatformViewController *vc = [[RXPayPlatformViewController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (UIButton *)settedBtn{
    if (!_settedBtn) {
        ...
        [_settedBtn setTitle:@"固定计费点支付示例" forState:normal];
...
- (UIButton *)customBtn{
    if (!_customBtn) {
        ...
        [_customBtn setTitle:@"选择计费点支付示例" forState:normal];
